...
class CustomerController extends CrudController
{
	...
	*/
	...
	public function actionUpdate($id)
	{
		...
		if(isset($_POST['Customer']))
		{
			...
			if($model->validate()&&$profile->validate()&&$this->validateAddress()) {
				...
				//$data = Address::model()->findAll('user_id=:user_id', array(':user_id'=>$model->id));
				//Address::model()->deleteAll('user_id=:user_id', array(':user_id'=>$model->id));
				foreach($this->_address as $address){
					$address->user_id = $model->id;
					$address->save();
				}
	...
	public function actionZones($id)
	{
		...
		if (Yii::app()->getRequest()->getIsAjaxRequest() && isset($id)){
			$data = Zone::model()->findAll('country_id=:country_id', 
					  array(':country_id'=>(int)$_POST['Address'][$id]['country_id']));
	...
	public function loadAddress($id){
		$data = Address::model()->findAll('user_id=:user_id', array(':user_id'=>$id));
		$this->_address = $data;
	}
	...
	public function validateAddress(){
		if(isset($_POST['Address']))
		{
			...
			foreach($_POST['Address'] as $i=>$item)
			{
				$model = new Address;
				if(isset($_POST['Address'][$i])){
					$model->attributes=$_POST['Address'][$i];
					$model->user_id=0;
