...
?>
	...
	<?php echo $form->errorSummary($model); ?>
		...
		<?php echo $form->dropDownList($model,"[$id]country_id", GxHtml::listDataEx(Country::model()->findAllAttributes(null, true)),
		array(
			'ajax' => array(
			'type'=>'POST', //request type
			'url'=>$this->createUrl('zones')."id/$id", //url to call.
			'update'=>'#Address_'.$id.'_zone_id',
		)));?>
		<?php echo $form->error($model,"[$id]country_id"); ?>
		</div><!-- row -->
		<div class="row">
		<?php echo $form->labelEx($model,"[$id]zone_id"); ?>
		<?php echo $form->dropDownList($model,"[$id]zone_id", array()); ?>
