...
abstract class BasePaymentTransaction extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('transaction_date, reference_number, payment_reference, approved_amount, response_description, response_code, transaction_amount, customer_name, order_id, query_date', 'required'),
			array('transaction_date, reference_number, payment_reference, customer_name', 'length', 'max'=>50),
			array('approved_amount, transaction_amount', 'length', 'max'=>10),
			array('response_description', 'length', 'max'=>100),
			array('response_code', 'length', 'max'=>5),
			array('order_id', 'length', 'max'=>20),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, transaction_date, reference_number, payment_reference, approved_amount, response_description, response_code, transaction_amount, customer_name, order_id, query_date', 'safe', 'on'=>'search'),
		);
	}
